...
async function generatePdfResume(employee) {
  return new Promise((resolve, reject) => {
    ...
    if (photoBuf) {
      try {
        doc.image(photoBuf, margin, y, { width: 80, height: 80 });
      } catch {}
      const nameX = margin + 90;
      const nameW = doc.page.width - margin * 2 - 95;

      doc.font(FONT_BOLD).fontSize(18).fillColor(...BRAND_DARK);
      doc.text(employee.name || '', nameX, y + 5, { width: nameW });
      y += doc.heightOfString(employee.name || '', { width: nameW }) + 10;

      doc.font(FONT_BOLD).fontSize(13).fillColor(...BRAND_ACCENT);

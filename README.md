...
- отдельные архивы сотрудников и проектов с восстановлением;
...
- почтовые рассылки по сотрудникам (назначение проектов выполняется без писем);
...
PostgreSQL доступен на хосте через порт `5433`, а приложение внутри Docker-сети подключается к нему по имени `postgres` и порту `5432`.

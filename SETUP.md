...
|---|---|---|
| `/login.html` | Вход в панель управления | администратор, скрам-мастер, служебная роль руководителя |
...
| `/projects.html` | Общий реестр проектов | только главный администратор |
| `/project.html` | Полная карточка отдельного проекта | только главный администратор |
| `/settings.html` | Настройки, пользователи, почта и шаблоны | набор разделов зависит от роли |
| `/form.html?token=...` | Индивидуальная анкета сотрудника | сотрудник или РП |
| `/myprojects.html?token=...` | Проекты, закреплённые за РП | сотрудник с признаком РП |
...
| Руководитель | `leader` | ограниченная служебная роль; не может проверять и редактировать данные сотрудников и не получает права РП автоматически |

### Сотрудники и РП

Сотрудники хранятся в таблице `employees` и открывают анкету по персональному токену без логина и пароля.

У сотрудника может быть признак `is_rp = true`. Такой сотрудник является руководителем проекта и видит кнопку «Мои проекты».

> Важно: роль панели `leader` и сотрудник с признаком `РП` — это разные сущности. Назначение пользователю роли `leader` не делает его РП. РП назначается из списка сотрудников на дашборде или в карточке проекта.

### Матрица прав

| Действие | Администратор | Скрам-мастер | `leader` | РП по ссылке | Сотрудник по ссылке |
|---|:---:|:---:|:---:|:---:|:---:|
| Вход в панель управления | ✓ | ✓ | ✓ | — | — |
| Просмотр списка сотрудников | ✓ | ✓ | — | только список команды | только свои данные |
| Поиск компетенций | ✓ | ✓ | — | — | — |
| Прямое редактирование сотрудника | ✓ | ✓ | — | — | — |
| Подтверждение и отклонение изменений | ✓ | ✓ | — | — | — |
| Назначение и снятие РП | ✓ | — | — | — | — |
| Общий реестр проектов | ✓ | — | — | только свои проекты | — |
| Создание проекта | ✓ | — | — | черновик для себя | — |
| Назначение РП проекту | ✓ | — | — | — | — |
| Изменение общей карточки проекта | ✓ | — | — | только закреплённого проекта | — |
| Изменение своей роли в проектном опыте | ✓ | ✓ | — | как участник | ✓ |
...
flowchart LR
    ...
    ADMIN -->|назначает признак РП| RP[Руководитель проекта]
...
- назначать выбранных сотрудников руководителями проектов;
- снимать признак РП;
...
РП — это действующий сотрудник, которому главный администратор установил признак «РП». После этого сотрудник получает доступ к разделу «Мои проекты»: он видит назначенные ему проекты либо может создать собственный проект-черновик.

РП работает по своей индивидуальной ссылке:

1. Открывает `/form.html?token=...`.
2. Нажимает «Мои проекты».
3. Видит только проекты, где его `employee_id` указан как `leader_employee_id`.
4. Открывает проект на редактирование.
5. Заполняет общие данные и состав команды.
...
РП также может создать новый проект-черновик для себя на странице «Мои проекты». Такой проект сразу закрепляется за ним.
...
- новым участникам отправляется письмо о добавлении в проект;
...
- фотографию;
- свою должность и роль внутри проектного опыта;
...
Эти поля отображаются как нередактируемые и дополнительно защищены на сервере. Даже ручная подмена запроса не изменит общие данные проекта.
...
| Проект отправлен РП | РП | перечень проектов и ссылка «Мои проекты» |
| Новый участник добавлен в проект | сотрудник | проект, РП и ссылка на личное портфолио |

...
| Руководитель проекта (РП) | `leader` | личный кабинет проектов; видит и редактирует только проекты, где связанному сотруднику назначена роль РП |

### Сотрудники и РП

Сотрудники хранятся в таблице `employees` и открывают анкету по персональному токену без логина и пароля.

У сотрудника может быть признак `is_rp = true`. Для входа РП главный администратор создаёт ему пользователя панели с ролью `leader` и связывает эту учётную запись с сотрудником.

> Роль `leader` обязательно связана с действующим сотрудником, отмеченным как РП. Это связывание определяет, какие проекты доступны после входа.
...
| Просмотр списка сотрудников | ✓ | ✓ | только при добавлении в команду | только свои данные |
| Поиск компетенций | ✓ | ✓ | — | — |
...
РП — это действующий сотрудник, которому главный администратор установил признак «РП» и создал связанную учётную запись панели с ролью «Руководитель проекта (РП)».

РП работает через личный кабинет:

1. Входит через `/login.html` по почте и паролю.
2. Автоматически попадает на `/projects.html`.
3. Видит только проекты, где его связанный `employee_id` указан как `leader_employee_id`.

...
Документ соответствует состоянию проекта на 9 сентября 2026 года.
...
4. [Работа администратора департамента](#работа-администратора-департамента)
5. [Работа ГСМ и скрам-мастера](#работа-гсм-и-скрам-мастера)
...
- РД просматривает данные, ГСМ и СМ ведут портфолио, РП управляет закреплёнными проектами, АдминД администрирует систему;
...
|---|---|---|
| `/login.html` | Вход в панель управления | РД, ГСМ, СМ, РП и АдминД |
| `/index.html` | Дашборд сотрудников и поиск | все роли панели; РД — только просмотр |
| `/review.html` | Проверка предложенных изменений | все роли панели; РД — только просмотр |
| `/history.html` | История решений и повторная обработка | все роли панели; РД — только просмотр |
| `/projects.html` | Реестр действующих проектов | РД, ГСМ, СМ и АдминД — общий; РП — только свои |
| `/project.html` | Карточка отдельного проекта | РД — просмотр; ГСМ/СМ/АдминД — все; РП — только свои |
| `/archive.html` | Архив сотрудников | ГСМ, СМ, РП и АдминД |
| `/settings.html` | Личный пароль и системные настройки | все роли — пароль; технические настройки — только АдминД |
...
|---|---|---|
| Руководитель департамента (РД) | `department_head` | просмотр сводной информации, сотрудников, изменений и проектов без изменения данных |
| Главный скрам-мастер (ГСМ) | `chief_scrum` | ведение портфолио сотрудников, проверка изменений и управление проектами |
| Скрам-мастер (СМ) | `scrum` | ведение профилей сотрудников, проверка изменений и управление проектами |
| Руководитель проекта (РП) | `leader` | права СМ по сотрудникам и управление только закреплёнными за ним проектами |
| Администратор департамента (АдминД) | `admin` | учётные записи, роли, справочники, импорт и технические настройки системы |

### Сотрудники, СМ и РП

Сотрудники хранятся в таблице `employees` и открывают анкету по персональному токену без логина и пароля.

Учётные записи СМ (`scrum`) и РП (`leader`) всегда связаны с портфолио сотрудника. При создании учётной записи система находит действующий профиль по ФИО и почте, а если профиля ещё нет — автоматически создаёт пустое портфолио. Для РП дополнительно устанавливается признак `is_rp = true`.

> Роли `scrum` и `leader` обязательно связаны с действующим сотрудником. Для РП эта связь также ограничивает список проектов только закреплёнными за ним карточками.

### Матрица прав

| Действие | РД | ГСМ | СМ | РП | АдминД | Сотрудник по ссылке |
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| Вход, дашборд, поиск и просмотр профилей | ✓ | ✓ | ✓ | ✓ | ✓ | только свои данные |
| Создание и редактирование сотрудников | — | ✓ | ✓ | ✓ | ✓ | предложение изменений в своём профиле |
| Проверка изменений | просмотр | ✓ | ✓ | ✓ | ✓ | — |
| Создание ссылки, архив и восстановление сотрудника | — | ✓ | ✓ | ✓ | ✓ | — |
| Безвозвратное удаление сотрудника | — | — | — | — | ✓ | — |
| Формирование DOCX, PDF и Excel | ✓ | ✓ | ✓ | ✓ | ✓ | — |
| Массовая рассылка | — | ✓ | ✓ | ✓ | ✓ | — |
| Просмотр проектов | все | все | все | только закреплённые | все | свои записи в проектном опыте |
| Создание и редактирование проектов | — | ✓ | ✓ | только закреплённые | ✓ | — |
| Импорт, должности, компетенции, учётные записи и технические настройки | — | — | — | — | ✓ | — |

## Общая схема взаимодействия

```mermaid
flowchart LR
    RD[Руководитель департамента] -->|просмотр| DASH[Дашборд сотрудников]
    GSM[Главный скрам-мастер] --> DASH
    SCRUM[Скрам-мастер] --> DASH
    RP[Руководитель проекта] --> DASH
    DASH --> SEARCH[Поиск компетенций]
    DASH --> REVIEW[Проверка изменений]
    AD[Администратор департамента] --> PROJECTS[Общий реестр проектов]
    AD --> SETTINGS[Настройки и пользователи]
    ...
    AD -->|создаёт учётную запись; профиль связывается по ФИО| RP
    ...
    DB --> PROJECTS
    DB --> FORM
    MAIL[SMTP] <-->|уведомления и ссылки| AD
    MAIL <-->|уведомления| EMP
    MAIL <-->|проекты| RP
```

## Работа администратора департамента

Администратор департамента входит по электронной почте и паролю через `/login.html`.
...
- создавать учётные записи СМ и РП; профиль автоматически находится по ФИО/email либо создаётся, а для РП признак устанавливается без действий на дашборде;
...
## Работа ГСМ и скрам-мастера

Главный скрам-мастер и скрам-мастер входят через `/login.html` и работают с портфолио сотрудников.
...
Кроме работы с сотрудниками, ГСМ и СМ могут создавать и редактировать проекты, а также назначать руководителей. Проекты не архивируются. Импорт, справочники, управление учётными записями и технические настройки доступны только АдминД.

## Работа руководителя проекта

РП — это отдельно назначаемая роль сотрудника, не связанная с его должностью. Для входа создаётся учётная запись панели с ролью `leader`; профиль связывается по ФИО или почте.

РП работает через общую панель:
...
3. Через вкладку «Мои проекты» открывает `/projects.html`.
4. Видит и изменяет только проекты, в которых назначен руководителем.
5. Открывает проект на редактирование, заполняет общие данные и состав команды.
6. Сохраняет карточку.

По сотрудникам РП имеет те же рабочие права, что ГСМ и СМ. По проектам его доступ ограничен закреплёнными карточками.
...
Импорт запускает пользователь панели кнопкой на вкладке «Проекты». Файл не загружается автоматически при запуске сайта.
...
Для отправки писем администратор департамента заполняет SMTP в настройках. Почта пользователя, вошедшего в панель, автоматически используется как адрес отправителя/ответа в разрешённых сценариях.
...
### 3. Исходная база (необязательно)

Если дампа нет, PostgreSQL запускается пустым, после чего приложение автоматически создаёт схему, начальные справочники и первую учётную запись администратора. Каталоги `data/` и `uploads/` также создаются автоматически, а служебный контейнер `storage-init` настраивает права записи.

Чтобы при первом запуске загрузить существующую базу, положите файл по пути `docker/postgres/seed/portfolio_backup.dump` до запуска Docker Compose.
...
AD_DEFAULT_ROLE=department_head

...
- ГСМ, СМ и РП работают с общим списком сотрудников и проектов;
...
| `/index.html` | Дашборд сотрудников, поиск и действия | ГСМ, СМ и РП |
| `/review.html` | Проверка предложенных изменений | ГСМ, СМ и РП |
| `/history.html` | История решений и повторная обработка | ГСМ, СМ и РП |
| `/projects.html` | Общий реестр действующих проектов | ГСМ, СМ и РП |
| `/project.html` | Полная карточка отдельного проекта | ГСМ, СМ и РП |
| `/archive.html` | Отдельный архив сотрудников | ГСМ, СМ и РП |
| `/projects-archive.html` | Отдельный архив проектов | ГСМ, СМ и РП |
| `/settings.html` | Настройки, пользователи, почта и шаблоны | ГСМ, СМ и РП |
...
|---|---|---|
| ГСМ | `admin` | полный доступ ко всем разделам и действиям |
| СМ | `scrum` | полный доступ ко всем разделам и действиям |
| РП | `leader` | полный доступ ко всем разделам и действиям |

### Сотрудники и РП

Сотрудники хранятся в таблице `employees` и открывают анкету по персональному токену без логина и пароля.

У сотрудника может быть отдельная роль РП (`is_rp = true`), которая не зависит от должности. Её можно назначить при создании сотрудника или через меню действий в его карточке. Для входа РП создаётся пользователь панели с ролью `leader`; система связывает его с профилем сотрудника по ФИО или почте.

> Роль `leader` связывается с действующим сотрудником, но доступ к разделам панели у ГСМ, СМ и РП одинаковый.
...
| Реестр проектов | все проекты | все проекты | все проекты | — |
| Создание проекта | ✓ | ✓ | ✓ | — |
| Назначение РП проекту | ✓ | ✓ | ✓ | — |
| Изменение общей карточки проекта | ✓ | ✓ | ✓ | — |
...
Кроме работы с сотрудниками, СМ имеет тот же доступ к проектам, архиву, настройкам, импорту и управлению пользователями, что ГСМ и РП.
...
3. Через вкладку «Проекты» открывает общий реестр `/projects.html`.
4. Видит тот же реестр, вкладки и действия, что ГСМ и СМ.
5. Открывает проект на редактирование, заполняет общие данные и состав команды.
6. Сохраняет карточку.

ГСМ, СМ и РП работают с единым набором страниц и одинаковыми правами.

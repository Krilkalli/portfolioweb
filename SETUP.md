...
- РД просматривает данные, ГСМ и СМ ведут портфолио, РП управляет закреплёнными проектами, АД администрирует систему;
...
|---|---|---|
| `/login.html` | Вход в панель управления | РД, ГСМ, СМ, РП и АД |
...
| `/projects.html` | Реестр действующих проектов | РД, ГСМ, СМ и АД — общий; РП — только свои |
| `/project.html` | Карточка отдельного проекта | РД — просмотр; ГСМ/СМ/АД — все; РП — только свои |
| `/archive.html` | Архив сотрудников | ГСМ, СМ, РП и АД |
| `/projects-archive.html` | Архив проектов | все роли панели; РД — только просмотр, РП — только свои |
| `/settings.html` | Личный пароль и системные настройки | все роли — пароль; технические настройки — только АД |
...
| Администратор департамента (АД) | `admin` | учётные записи, роли, справочники, импорт и технические настройки системы |

### Сотрудники и РП

Сотрудники хранятся в таблице `employees` и открывают анкету по персональному токену без логина и пароля.

У сотрудника может быть отдельная роль РП (`is_rp = true`), которая не зависит от должности. Администратор департамента создаёт пользователю учётную запись с ролью `leader`; система связывает её с профилем сотрудника по ФИО или почте и автоматически устанавливает признак РП.

> Роль `leader` обязательно связывается с действующим сотрудником. Эта связь ограничивает список проектов РП только закреплёнными за ним карточками.

### Матрица прав

| Действие | РД | ГСМ | СМ | РП | АД | Сотрудник по ссылке |
|---|:---:|:---:|:---:|:---:|:---:|:---:|
...
- архивировать и восстанавливать проекты без потери данных и участников;
...
Кроме работы с сотрудниками, ГСМ и СМ могут создавать, редактировать, назначать руководителей, архивировать и восстанавливать проекты. Импорт, справочники, управление учётными записями и технические настройки доступны только АД.
